...
<table cellpadding="0" cellspacing="0" style="max-width:560px;width:100%">
  ...
  <tr>
    <td style="background:#ffffff;padding:40px;border-left:1px solid #e2e8f0;border-right:1px solid #e2e8f0">
      <p style="margin:0 0 20px;font-size:15px;color:#334155;line-height:1.7">
        L'utente <strong>{$userEmail}</strong> ha effettuato il primo accesso al QR Generator.
      </p>
      <p style="margin:0;font-size:14px;color:#64748b;line-height:1.7">
  ...
  <tr>
    <td align="center"
        style="background:#f8fafc;border:1px solid #e2e8f0;border-top:none;
               border-radius:0 0 12px 12px;padding:20px 40px">
      <p style="margin:0;font-size:12px;color:#94a3b8;line-height:1.6">
        Notifica automatica — QR Generator
      </p>
    </td>
  </tr>
...
HTML;
    ...
    $plain = "Nuovo primo accesso — QR Generator\n\n"
           . "L'utente {$userEmail} ha effettuato il primo accesso al QR Generator.\n\n"
    ...
    try {
        ...
        $mail->Subject  = "Nuovo accesso — QR Generator: {$userEmail}";
        $mail->isHTML(true);
        $mail->Body     = $html;
        $mail->AltBody  = $plain;
        $mail->send();
    } catch (Throwable) {
        // La notifica admin non deve mai bloccare l'applicazione
    }
}
